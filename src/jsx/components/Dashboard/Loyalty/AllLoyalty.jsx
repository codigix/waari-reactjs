...
const AllLoyalty = () => {
    ...
    const getNumberOfGuests = async () => {
        try {
            setIsTableLoading(true);
            const response = await get(`/all-referee-no-of-guests`);
    ...
    const getGuestSalesReceived = async () => {
        try {
            setIsTableLoading(true);
            const response = await get(`/all-guests-sales-received`);
            setSalesReceived(response.data.refereeGuestsSales);
            setIsTableLoading(false);
        } catch (error) {
            setIsTableLoading(false);
    ...
    const getResults = async () => {
        try {
            ...
            const response = await get(
                `/all-loyalty-guests?name=${guestName}&cardName=${
                    loyaltyCard?.value || ""
    ...
    useEffect(() => {
        hasComponentPermission(permissions, 295) && getNumberOfGuests();

        hasComponentPermission(permissions, 295) && getGuestSalesReceived();
    }, []);

    useEffect(() => {
        hasComponentPermission(permissions, 295) && getResults();
    }, [page, perPageItem]);
                ...
                <div className="col-lg-12" style={{ paddingTop: "40px" }}>
                    ...
                    <div className="card">
                        <div className="card-body">
                            ...
                            <div className="row">
                                {hasComponentPermission(permissions, 295) && (
