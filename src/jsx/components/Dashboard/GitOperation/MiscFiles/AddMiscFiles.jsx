...
const AddImageForm = ({tourId, handleSucessAdd}) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                const response = await post("/add-gt-miscellaneous-files-details", {...values, groupTourId: tourId});
                toast.success("Miscellaneous files added successfully");
                handleSucessAdd(true)
                setIsLoading(false);
            } catch (error) {
                setIsLoading(false);
                console.error(error);
    ...
    const handleFileUpload = async (event, fieldName) => {
        ...
        formik.setFieldValue(fieldName, [...formik.values[fieldName], ...uploadedUrls]);
    };

    useEffect(() => {
        let element = document.getElementById("misc-files-list");
        if (element) {
    ...
    return (
        <form className="card p-4" onSubmit={formik.handleSubmit}>
            ...
            <div className="col-lg-12 d-flex justify-content-between mt-3">
                <div onClick={handleSucessAdd} type="button" className="btn btn-back">
                    Back
                </div>
