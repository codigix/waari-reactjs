...
const BookingGrouptourRecord = () => {
    ...
    const columns = [
        ...
        hasComponentPermission(permissions, 225) && {
            title: "Billing",
            render: (item) => (
                <>
                    <div className="d-flex justify-content-center">
                        <Link
                            to={`/all-booking-enquiry/${item?.enquiryId}/${item.familyHeadGtId}`}
                            className="btn-tick"
                        >
                            <Tooltip title="View">
                                <svg
    ...
    const getViewgrouplist = async () => {
        try {
            ...
            const response = await get(
                `/all-booking-records-gt?perPage=${perPageItem}&page=${page}&guestName=${guestname}&tourTypeId=${
                    tourType?.value || ""
                }&destinationId=${
                    destination?.value || ""
                }&tourName=${tourName}&bookingDateFrom=${bookingDateFrom}&bookingDateTo=${bookingDateTo}&startDate=${travelDateFrom}&endDate=${travelDateTo}`
            );
            setIsLoading(false);
    ...
    useEffect(() => {
        hasComponentPermission(permissions, 224) && getTourTypeList();
        hasComponentPermission(permissions, 224) && getDestinationList();
    }, []);

    useEffect(() => {
        hasComponentPermission(permissions, 224) && getViewgrouplist();
    }, [page, perPageItem]);
    //to end booking record list
    return (
        <>
            {hasComponentPermission(permissions, 224) && (
                <>
                    <div className="row">
                            ...
                            <div className="card">
                                <div className="row page-titles mx-0 fixed-top-breadcrumb">
                                    <ol className="breadcrumb">
                                        ...
                                        <li className="breadcrumb-item  ">
                                            <Link to="/all-booking-group-tour-new">
                                                All Group Tour
                                            </Link>
                                        </li>
                                    </ol>
...
export default BookingGrouptourRecord;
