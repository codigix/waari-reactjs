	...
	const getFamilyMembersList = async () => {
		try {
			...
			const tranformedFamilyMembersList = response.data?.data?.map(
				(member) => ({
					groupGuestDetailId: member.groupGuestDetailId,
	...
	const getCancelledMembersList = async () => {
		try {
			...
			const reponse = await get(
				`/get-cancellation-process-data?enquiryGroupId=${enquiryId}&familyHeadGtId=${familyHead.familyHeadGtId}`
			);
					...
					<div className="col-md-5">
						{familyMembersList.map((member, index) => (
							<div className="row mb-2">
								...
								{!member.isCancel &&
								hasComponentPermission(permissions, 181) ? (
									<div className="col-md-6 col-sm-6 col-lg-6 col-12">
										<button
