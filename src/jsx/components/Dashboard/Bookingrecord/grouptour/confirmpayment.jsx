...
const confirmpayment = ({ enquiryId, familyHeadGtId }) => {
  ...
  const getFamilyHeadsList = async () => {
    try {
      ...
      getPaymentDetails(transformedFamilyHeadsList[0].familyHeadGtId);
    } catch (error) {
      console.log(error);
    } finally {
      setIsLoading(false);
  ...
  // 			`/view-payment-bill-gt?familyHeadGtId=${selectedFamilyHeadId ? selectedFamilyHeadId : familyHeadGtId}&enquiryGroupId=${enquiryId}`
  // 		);
  ...
  const getPaymentDetails = async (id) => {
    try {
      setIsLoading(true);
      const response = await get(
        `/view-payment-bill-gt?familyHeadGtId=${id}&enquiryGroupId=${enquiryId}`
      );
                      ...
                      familyHeadsList.map((item) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            getPaymentDetails(item.familyHeadGtId); // <-- fetch new data
                          }}
                        >
                          <div
                            className={`nav-link  ${
