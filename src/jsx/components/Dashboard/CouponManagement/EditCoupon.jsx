...
const validationSchema = Yup.object().shape({
  ...
  maxDiscount: Yup.string().when("discountType", {
    is: (discountType) => discountType?.value == 2,
    then: Yup.string().required("Enter The Max Discount Amount"),
    // otherwise: Yup.string(),
  }),
});
function EditCoupon() {
  ...
  // get coupon deatils for editing
  const getCouponsDetails = async () => {
    try {
        const result = await get(`coupon-data?couponId=${id}`)
        const { couponName, fromDate, toDate, discountType, discountValue, maxDiscount, status, isType } = result?.data?.data
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const result = await post("edit-coupon-info", data);
        ...
        <div className="col-lg-12" style={{ paddingTop: '40px' }}>
          <div className="card">
            <div className="row page-titles mx-0 fixed-top-breadcrumb">
                ...
                <li className="breadcrumb-item  ">
                  <Link to="javascript:void(0)">Edit Coupon</Link>
                </li>
              </ol>
            </div>
          </div>
          ...
          <div className="card-body">
            <div className="card-header mb-2 pt-0" style={{paddingLeft:"0"}}>
              <div className="card-title h5">Edit Coupon</div>
            </div>
