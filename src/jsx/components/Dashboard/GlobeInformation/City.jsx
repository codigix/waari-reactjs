...
const City = () => {
    ...
    const columns = [
        {
            title: "Sr.No.",
            render: (item, index) => <>{page * perPageItem - perPageItem + (index + 1)}</>,
            width: 40,
        },
        {
            title: "continental",
        ...
        hasComponentPermission(permissions, 61) || hasComponentPermission(permissions, 62)
            ? {
                  title: "Action",
                  render: (item) => (
    ...
    const getCityList = async () => {
        try {
            setIsLoading(true);
            const response = await get(`/all-city-list?page=${page}&perPage=${perPageItem}`);
