...
const Country = () => {
  ...
  const validation = useFormik({
    ...
    //     continent: Yup.string().required("Please Select Continent"),
    //     country: Yup.string()
    ...
    validationSchema: Yup.object({
      ...
      country: Yup.string(), // optional (no digit restriction)
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        setIsLoading(true);
        const response = await post(`add-country`, data);
        hasComponentPermission(permissions, 59) && getCountryList();
            ...
            >
              ...
              {hasComponentPermission(permissions, 62) && (
                <div className="d-flex justify-content-center">
                  <Link
                    className="btn-trash"
                    onClick={() => handleOpenDelete(item?.countryId)}
                  >
  ...
  const getCountryList = async () => {
    try {
      ...
      const response = await get(
        `/all-country-list?page=${page}&perPage=${perPageItem}`
      );
  ...
  const handleDelete = async () => {
    try {
      setIsLoading(true);
      const responseData = await get(`/delete-country?countryId=${deleteId}`);
      setOpen(false);
      setIsLoading(false);
      getCountryList();
  ...
  // To get the data for updating country start

  // To get the data for updating country end
  ...
  const validationUpdate = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        setIsLoading(true);
        const response = await post(`edit-country`, data);
        setIsLoading(false);
        toast.success(response?.data?.message);
                ...
                <div className="col-md-6 col-lg-6 col-sm-6 col-12">
                  <div className="mb-3">
                    ...
                    <Select
                      ...
                      value={continent.find(
                        (option) => option.value === validation.values.continent
                      )}
                    />
              ...
              <div>
                <div className="lg:mb-3 mb-3 md:mb-3 form-group">
                  ...
                  <input
                    ...
                    value={validationUpdate.values.countryName}
                  />
                  {validationUpdate.touched.countryName &&
                  validationUpdate.errors.countryName ? (
                    <span className="error">
                      {validationUpdate.errors.countryName}
                    </span>
                  ) : null}
