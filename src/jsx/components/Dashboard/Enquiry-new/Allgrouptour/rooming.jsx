...
const Rooming = ({
	...
	const validation = useFormik({
		...
		onSubmit: async (values) => {
			...
			if (totalRoomCount !== maxRoomCountFromProps) {
				toast.error(
					"Total room count must match number of pax for this family head."
				);
				return;
			}
	...
	const handleRoomCountChange = (countValue, index) => {
		validation.setFieldValue(`roomsArray[${index}].count`, Number(countValue));
	};
					...
					<div className="row">
						{validation.values.roomsArray.length > 0 &&
							validation.values.roomsArray.map((room, index) => (
								<div
									className="col-lg-4 col-md-6 col-sm-6 col-xs-12 mb-2"
									key={room.roomShareId}
								>
									<div className="row">
										<div className="col-md-6 col-lg-6 col-sm-12">
											<label className="text-label">{room.roomShareName}</label>
										</div>
										<div className="col-md-6 col-lg-6 col-sm-12">
											<div className="adult-room">
												...
												<input
													...
													value={room.count}
													onChange={(e, index) =>
														handleRoomCountChange(e.target.value, index)
													}
												/>
												<button
													type="button"
