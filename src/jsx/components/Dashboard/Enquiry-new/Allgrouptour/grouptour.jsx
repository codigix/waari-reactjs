...
const Guesttour = () => {
	...
	const todaysFollowUp = async () => {
		try {
			...
			const response = await get(
				`/all-enquiry-today-list-gt?perPage=${perPageItem}&page=${page}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 202) && todaysFollowUp();
	}, [page, perPageItem]);
	...
	const expiredFollowUp = async () => {
		try {
			...
			const response = await get(
				`/all-enquiry-expired-list-gt?perPage=${expiredPerPageItem}&page=${expiredPage}`
			);
	...
	const UpcomingFollowUp = async () => {
		try {
			...
			const response = await get(
				`/all-enquiry-upcoming-list-gt?perPage=${perPageItemUpcoming}&page=${pageUpcoming}&search=${searchUpcoming}&tourName=${tourNameUpcoming}&startDate=${startDateUpcoming}&endDate=${endDateUpcoming}`
			);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 202) && UpcomingFollowUp();
					...
					<div className="card">
						...
						>
							<ol className="breadcrumb">
								...
								<li className="breadcrumb-item  ">
									<Link to="/all-group-tour-new">Group Tour</Link>
								</li>
							</ol>
						</div>
					</div>

					<div className="card d-flex justify-content-start align-items-center flex-wrap mb-2">
					...
					<div className="d-flex">
						...
						<div className="card">
							<div className="card-body">
								...
								{hasComponentPermission(permissions, 202) && (
									<>
										<Table
