...
import { updateAdvancePaymentStatus } from "../../../../../store/actions/groupTourAction";
import { NoImage } from "../../../../utils/assetsPaths";

const PaymentCT = ({ enquiryId, familyHead }) => {
	const dispatch = useDispatch();

	const { isPaymentDone } = useSelector((state) => state.groupTour);
	const [alreadyFilledForm, setAlreadyFilledForm] = useState(true);
	...
	const [sameAsCheckData, setSameAsCheckData] = useState({
		billingName: "",
		address: "",
		phoneno: "",
		pan: ""
	...
	const [samePan, setSamePan] = useState("");
	...
	const showSameBillingName = async (event) => {
		if (event.target.checked) {
			validation.setFieldValue("billingname", sameAsCheckData.billingName);
		} else {
	...
	const showSameAddress = async (event) => {
		if (event.target.checked) {
			validation.setFieldValue("address", sameAsCheckData.address);
		} else {
	...
	const showSameContact = async (event) => {
		if (event.target.checked) {
			validation.setFieldValue("phoneno", sameAsCheckData.phoneno);
		} else {
	...
	const showSamePanNo = async (event) => {
		if (event.target.checked) {
			validation.setFieldValue("pan", sameAsCheckData.pan);
		} else {
			validation.setFieldValue("pan", "");
		}
		setSamePan(!samePan);
	};
	...
	const getPaymentDetails = async () => {
		try {
			...
			setSameAsCheckData({
				billingName: data.sameAsbillingName,
				address: data.sameAsaddress,
				phoneno: data.sameAsphoneno,
				pan: data.sameAsPanNo,
			});
			...
			setAlreadyFilledForm(data.transactionProof?.length);
	...
	const validation = useFormik({
		...
		validationSchema: Yup.object({
			...
			advanced: Yup.string()
				.required("Enter The Advance Payment Amount")
				.test({
					...
					message: "Advance payment amount must be positive and above 0.",
				}),
		...
		onSubmit: async (values) => {
			...
			try {
				...
				const response = await post(`/payment-calculation-ct`, data);
				toast.success(response?.data?.message);
				// navigate("/customized-tour-new");
				setAlreadyFilledForm(true);
				!isPaymentDone && dispatch(updateAdvancePaymentStatus(true));
	...
	// to-words in amount
	const amountInWords = Number(validation.values.advanced)
		...
		<section>
			...
			>
				...
				<div className="row mb-2 form-group">
					<div className="col-md-2">
						<label className="text-label">
							Phone number<span className="error-star">*</span>
						</label>
					</div>
					<div className="col-md-6">
						<input
							...
							value={
								sameContact
									? sameAsCheckData.phoneno
									: validation.values.phoneno
							}
						/>
						{validation.touched.phoneno && validation.errors.phoneno ? (
							<span className="error">{validation.errors.phoneno}</span>
						) : null}
					</div>
					{!alreadyFilledForm && sameAsCheckData.phoneno ? (
						<div className="col-sm-4">
							<div className="form-check mb-2">
								<input
				...
				<div className="row mb-2 form-group">
					...
					<div className="col-md-6">
						<input
							...
							onChange={samePan ? null : validation.handleChange}
							onBlur={samePan ? null : validation.handleBlur}
							value={
								samePan
									? sameAsCheckData.pan
									: validation.values.pan
							}
						/>
					...
					{!alreadyFilledForm && sameAsCheckData.pan ? (
						<div className="col-sm-4">
							<div className="form-check mb-2">
								<input
									onChange={showSamePanNo}
									type="checkbox"
									className="form-check-input"
									id="check4"
									value=""
