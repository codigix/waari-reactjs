...
const Customizedtour = () => {
					...
					<>
						<div className="d-flex justify-content-center">
							{hasComponentPermission(permissions, 28) && (
								<span
									className=""
									onClick={() => navigate(`/enquiry-ct/${item.enquiryCustomId}`)}
								>
									<Link className="btn-edit-user  me-2">
										<Tooltip title="Edit">
											<svg
	...
	const expiredFollowUp = async () => {
		try {
			...
			const response = await get(
				`/expired-enquiry-follow-CT?perPage=${expiredPerPageItem}&page=${expiredPage}`
			);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 32) && expiredFollowUp();
	...
	useEffect(() => {
		hasComponentPermission(permissions, 32) && UpcomingFollowUp();
						...
						<div className="card">
							<div className="row page-titles mx-0 fixed-top-breadcrumb">
								<ol className="breadcrumb">
									...
									<li className="breadcrumb-item  ">
										<Link to="/customized-tour-new">Customized Tour</Link>
									</li>
								</ol>
							</div>
						</div>
