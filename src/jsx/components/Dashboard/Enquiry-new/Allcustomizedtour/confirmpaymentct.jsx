...
const ConfirmPaymentCT = ({ enquiryId }) => {
	...
	const [familyHeadTab, setFamilyHeadTab] = useState(
		familyHeadsList.find((fHead) => fHead.status == 1)?.path
	);
	const [enquiryDetailCustomId, setEnquiryDetailCustomId] = useState(
		familyHeadsList.find((fHead) => fHead.status == 1)?.enquiryDetailCustomId
	);
	...
	const getFamilyHeadsList = async () => {
		try {
			...
			setFamilyHeadTab(
				transformedFamilyHeadsList.find((fHead) => fHead.isPaymentDone)?.path
			);

			setEnquiryDetailCustomId(
				transformedFamilyHeadsList.find((fHead) => fHead.isPaymentDone)
