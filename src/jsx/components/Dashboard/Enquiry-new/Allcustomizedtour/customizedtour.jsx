...
const Customizedtour = () => {
	...
	const columns = [
		...
		hasComponentPermission(permissions, 239) ||
							...
							<div className="d-flex justify-content-center">
								{hasComponentPermission(permissions, 239) && (
									<span
										className=""
										onClick={() => {
											navigate(`/all-enquiry-ct/${item.enquiryCustomId}`);
										}}
									>``
	...
	const todaysFollowUp = async () => {
		try {
			...
			const response = await get(
				`/all-enq-today-ct?perPage=${perPageItem}&page=${page}`
			);
			setIsLoading1(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 237) && todaysFollowUp();
	}, [page, perPageItem]);
	...
	const UpcomingFollowUp = async () => {
		try {
			setIsLoading(true);
			const response = await get(
				`/all-enq-upcoming-ct?perPage=${perPageItemUpcoming}&page=${pageUpcoming}&guestName=${search1}&groupName=${tourName1}&startDate=${startDate1}&endDate=${endDate1}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 237) && UpcomingFollowUp();
						...
						<>
							<div className="d-flex justify-content-center">
								{hasComponentPermission(permissions, 28) && (
									<span
										className=""
										onClick={() => navigate(`/all-enquiry-ct/${item.enquiryCustomId}`)}
									>
										<Link className="btn-edit-user  me-2">
										<Tooltip title="Edit">
											<svg
	...
	const expiredFollowUp = async () => {
		try {
			...
			const response = await get(
				`/all-enq-expired-ct?perPage=${expiredPerPageItem}&page=${expiredPage}`
			);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 237) && expiredFollowUp();
			...
			<div className="row">
					...
					<div className="card">
						<div className="row page-titles mx-0 fixed-top-breadcrumb">
							   <ol className="breadcrumb">
								...
								<li className="breadcrumb-item  ">
									<Link to="/all-customized-tour-new">Customized Tour</Link>
								</li>
							</ol>
						</div>
					</div>

					<div className="card d-flex justify-content-between align-items-center flex-wrap mb-2">

					<h1 className="card-header">Dashboard / Enquiry Followup / Custom Tour</h1>
					</div>
				...
				<div className="card">
						<div className="card-body">
							...
							{hasComponentPermission(permissions, 237) && (
								<>
