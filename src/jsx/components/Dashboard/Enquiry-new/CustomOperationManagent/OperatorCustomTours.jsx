...
const OperatorCustomTours = () => {
							...
							<div className="d-flex justify-content-center">
								...
								{hasComponentPermission(permissions, 365) && (
									<span
										className=""
										onClick={() => {
											navigate(`/operation-management/${item.enquiryCustomId}`);
										}}
									>
	...
	const todaysFollowUp = async () => {
		try {
			...
			const response = await get(
				`/assigned-custom-tour-list?perPage=${perPageItem}&page=${page}`
			);
			setIsLoading1(false);
	...
	useEffect(() => {
	 hasComponentPermission(permissions, 353) &&	todaysFollowUp();
	}, [page, perPageItem]);
						...
						<>
							<div className="d-flex justify-content-center">
								{hasComponentPermission(permissions, 365) && (
									<span
										className=""
										onClick={() => navigate(`/operation-management/${item.enquiryCustomId}`)}
									>
										<Link className="btn-edit-user  me-2">
										<Tooltip title="Edit">
											<svg
	...
	const expiredFollowUp = async () => {
		try {
			...
			const response = await get(
				`/assigned-expired-custom-tour-list?perPage=${expiredPerPageItem}&page=${expiredPage}`
			);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 353) && expiredFollowUp();
	...
	const UpcomingFollowUp = async () => {
		try {
			setIsLoading(true);
			const response = await get(
				`/assigned-upcoming-custom-tour-list?perPage=${perPageItemUpcoming}&page=${pageUpcoming}&guestName=${search1}&groupName=${tourName1}&startDate=${startDate1}&endDate=${endDate1}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
    hasComponentPermission(permissions, 353) && UpcomingFollowUp();
				...
				<div className="card">
					...
					<div className="card">
						<div className="card-body">
							...
							{hasComponentPermission(permissions, 353) && (
								<>
...
export default OperatorCustomTours;
