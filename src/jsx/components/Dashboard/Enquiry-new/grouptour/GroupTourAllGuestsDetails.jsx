...
const GroupTourAllGuestsdetails = () => {
	...
	useEffect(() => {
		const getGuestsDetails = async () => {
			const response = await get(
				`/get-guest-details-gt?familyHeadGtId=${familyHeadGtId}&enquiryGroupId=${enquiryId}`
			);
	...
	useEffect(() => {
        ...
        // console.log(path);
        let element = document.getElementById("search-group-tour-all-guests");
        // console.log(element);
        if (element) {
...
export default GroupTourAllGuestsdetails;
