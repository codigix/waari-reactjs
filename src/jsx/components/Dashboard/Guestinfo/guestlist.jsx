...
const Guestlist = () => {
	...
	const getGuestList = async () => {
		try {
			...
			const response = await get(
				`guests-list?page=${page}&perPage=${perPageItem}&guestName=${guestName}&cardName=${
					loyaltyCard?.value || ""
				}`
			);
			...
			render: (item) => (
				<>
					<div className="d-flex justify-content-center">
						{hasComponentPermission(permissions, 112) && (
							<Link
								to={`/edit-guest-new/${item.userId}`}
						...
						<Link to={`/view-guest/${item.guestId}`} className="btn-tick">
							<Tooltip title="View">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									height="1em"
	...
	useEffect(() => {
		hasComponentPermission(permissions, 100) && getGuestList();
	}, [page, perPageItem]);
		...
		<>
			<div className="row">
				<div className="col-lg-12" style={{ paddingTop: '40px' }}>
					...
					{hasComponentPermission(permissions, 141) && (
						<div className="card">
							<div className="row">
					...
					{hasComponentPermission(permissions, 100) && (
						<div className="card">
