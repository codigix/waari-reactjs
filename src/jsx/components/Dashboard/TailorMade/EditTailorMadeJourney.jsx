...
function EditTailorMadeJourney() {
    const validationSchema = Yup.object().shape({
        tourdurationnights: Yup.number()
            .required("Total Duration Nights is required"),
        tourdurationdays: Yup.number()
            .required("Total Duration Days is required")
            .min(1, "Minimum 1 day duration is required"),
        ...
        tailormadeinclusions: Yup.array().of(
            Yup.object().shape({
                description: Yup.string().required("Enter the Inclusion Desription"),
            })
        ),
        tailormadeexclusions: Yup.array().of(
            Yup.object().shape({
                description: Yup.string().required("Enter the exclusion Desription"),
            })
        ),
    ...
    const getTourDetails = async () => {
        try {
            const response = await get(`/get-edit-tailor-made?tailorMadeId=${id}`);
            ...
            formik.setFieldValue("uniqueExperience", response.data.data.uniqueExperience);
            ...
            formik.setFieldValue(
                "hotelprice",
                transformTourPriceData(response.data.data.tourPrice)
            );

            formik.setFieldValue("tailormadeinclusions", response.data.data.tailormadeinclusions);
            formik.setFieldValue("tailormadeexclusions", response.data.data.tailormadeexclusions);
            ...
            formik.setFieldValue("visaInstruction", response.data.data.visaInstruction);
            formik.setFieldValue("visaAlerts", response.data.data.visaAlerts);
            formik.setFieldValue("insuranceDetails", response.data.data.insuranceDetails);
            formik.setFieldValue("euroTrainDetails", response.data.data.euroTrainDetails);
            formik.setFieldValue("nriOriForDetails", response.data.data.nriOriForDetails);
            ...
            formik.setFieldValue("websiteBanner", response.data.data.websiteBanner || "");
            formik.setFieldValue("websiteDescription", response.data.data.websiteDescription || "");
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const addDetailsItenerary = (dayDifference) => {
        formik.setFieldValue(
            "detailIntenirary",
            Array.from({ length: dayDifference }, () => ({ ...detailedItineraryObj }))
        );
    };
    ...
    const handleAddImage = (index) => {
        formik.setFieldValue(`detailIntenirary[${index}].tailorMadeitineraryimages`, [
    ...
    const handleRemoveImage = (index, subIndex) => {
        ...
        formik.setFieldValue(`detailIntenirary[${index}].tailorMadeitineraryimages`, updatedImages);
    };

    // Transform API response to match formik state structure
    const transformTourPriceData = (tourPrice) => {
        // Initialize the structure for each type
        const hotelPriceStructure = [
            { type: 0, hotels: [] }, // Deluxe
            { type: 1, hotels: [] }, // Super Deluxe
            { type: 2, hotels: [] }, // Premium
        ];

        // Iterate over the tourPrice array and populate the structure
        tourPrice.forEach((item) => {
            const { type, hotelName, tourPrice, offerPrice, commissionPrice } = item;

            // Find the corresponding type group
            const typeGroup = hotelPriceStructure.find((group) => group.type === type);
            if (typeGroup) {
                // Add the hotel data to the corresponding type group
                typeGroup.hotels.push({
                    hotelName,
                    tourPrice: parseFloat(tourPrice), // Ensure numeric value
                    offerPrice: parseFloat(offerPrice), // Ensure numeric value
                    commissionPrice: parseFloat(commissionPrice), // Ensure numeric value
                });
            }
        });

        return hotelPriceStructure;
    };
        ...
        onSubmit: async (values) => {
            ...
            const detailsIternary = values.detailIntenirary.map((item, index) => {
                return {
                    ...
                    tailormadeitineraryimagesList: item.tailorMadeitineraryimages.map((image) => ({
            ...
            try {
                setIsLoading(true);
                const response = await post(`update-tailor-made-list?tailorMadeId=${id}`, data);
                setIsLoading(false);
    ...
    const isValidImageSize = (size, fieldName) => {
        ...
        if (fieldName === "bgImage") {
            return (
                width === requiredSizeForBgImage.width && height === requiredSizeForBgImage.height
            );
    ...
    const handleDaysChange = (e) => {
        ...
        if (!isNaN(days.toString())) {
            ...
            addDetailsItenerary(Number(length));
        }
    };
            ...
            <div className="row">
                <div className="col-lg-12">
                    <form onSubmit={formik.handleSubmit} className="needs-validation">
                        ...
                        {formik.values.tourdurationdays && formik.values.tourdurationdays > 0 && (
                            <div className="card">
                                <div className="card-body">
                                    <div className="row">
                                        <div className="col-md-12">
                                        ...
                                        <div v className="col-md-6">
                                            ...
                                            <div className="col-md-12">
                                                <div className="Neon Neon-theme-dragdropbox bg-gt">
                                                    ...
                                                    />
                                                    <div className="Neon-input-dragDrop">
                                                        {formik.values.websiteBanner?.length ==
                                                        0 ? (
                                                            <div className="Neon-input-inner ">
                                                                <div className="Neon-input-text">
                                                                    <svg
...
export default EditTailorMadeJourney;
