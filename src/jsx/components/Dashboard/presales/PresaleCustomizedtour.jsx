...
const PresaleCustomizedtour = () => {
	...
	const columns = [
		...
		{
			title: "No Of TravelPeople",
			dataIndex: "noOfTravelPeople",
			key: "noOfTravelPeople",
			width: 80,
		},

		{
			title: "Spend Amount Per Person",
			dataIndex: "nextFollowUp",
			key: "nextFollowUp",
			width: 80,
			render: item => (
				<div>
					{Number(item.pricePerPersonMin)?.toFixed(0)	+ "-" + Number(item.pricePerPersonMax)?.toFixed(0)}
				</div>
			)
		},
		{
		...
		hasComponentPermission(permissions, 304) 
			? {
					title: "Assign",
					render: (item) => (
						<>
							<div className="d-flex justify-content-center">
								...
								{hasComponentPermission(permissions, 304) && (
									<span
										className=""
										onClick={() => {
											navigate(`/assign-ct-enquiry/${item.id}`);
										}}
									>
...
// 										className=""
// 										onClick={() => {
// 											navigate(`/enquiry-ct/${item.enquiryCustomId}`);
// 										}}
// 									>
// 										<Link className="btn-edit-user  me-2">
// 										<Tooltip title="Edit">
...
// 	);
	...
	const todaysFollowUp = async () => {
		try {
			...
			const response = await get(
				`/plan-enq-users-list-ct?perPage=${perPageItem}&page=${page}&search=${search}&tourName=${tourName}&startDate=${startDate}&endDate=${endDate}`
			);
			setIsLoading1(false);
	...
	useEffect(() => {
	 hasComponentPermission(permissions, 305) &&	todaysFollowUp();
	}, [page, perPageItem]);
	...
	// useEffect(() => {
    // hasComponentPermission(permissions, 305) && UpcomingFollowUp();
					...
					<div className="card">
						<div className="row page-titles mx-0 fixed-top-breadcrumb">
							<ol className="breadcrumb">
								...
								<li className="breadcrumb-item  ">
									<Link to="/presale-customized-tour-new">Customized Tour</Link>
								</li>
							</ol>
						</div>
					</div>
					...
					<div className="card">
						<div className="card-body">
							<div className="card-header mb-3" style={{ padding: "0" }}>
								<div className="card-title h5">Presales Tailor made Enquiries</div>
							</div>

							

							{hasComponentPermission(permissions, 305) && (
								<>
...
export default PresaleCustomizedtour;
