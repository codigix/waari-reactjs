...
function EditGroupTour() {
    const validationSchema = Yup.object().shape({
        ...
        flights: Yup.array().when("departure", {
            is: (departure) => departure?.value == "1",
            then: Yup.array().of(
                Yup.object().shape({
                    flight: Yup.string(),
                    airline: Yup.string(),
            ...
            otherwise: Yup.array().of(
                Yup.object().shape({
                    ...
                    from: Yup.string()
                        ...
                        .matches(/^[a-zA-Z\s]+$/, "From must contain only letters and spaces"),
                    fromDate: Yup.string().required("From Date is required"),
                    fromTime: Yup.string().required("From Time is required"),
                    to: Yup.string()
                        .required("To is required")
                        .matches(/^[a-zA-Z\s]+$/, "To must contain only letters and spaces"),
    ...
    const getTourDetails = async () => {
        try {
            ...
            formik.setFieldValue("pickUpMeetTime", response.data.data.dtod[0].pickUpMeetTime);
            formik.setFieldValue("endCity", response.data.data.dtod[0].endCity);
            formik.setFieldValue("dropOffPoint", response.data.data.dtod[0].dropOffPoint);
            formik.setFieldValue("pickUpMeet", response.data.data.dtod[0].pickUpMeet);
            formik.setFieldValue("arriveBefore", response.data.data.dtod[0].arriveBefore);
            formik.setFieldValue("dropOffTime", response.data.data.dtod[0].dropOffTime);
    ...
    const handleStartDateChange = (e) => {
        ...
        const nightDifference = Math.floor((endDate - startDate) / (24 * 3600 * 1000));
    ...
    const handleRemoveImage = (index, subIndex) => {
        const updatedImages = formik.values.detailIntenirary[index].grouptouritineraryimages.filter(
            (_, i) => i !== subIndex
        );
        formik.setFieldValue(`detailIntenirary[${index}].grouptouritineraryimages`, updatedImages);
    };
        ...
        onSubmit: async (values) => {
            ...
            const detailsIternary = values.detailIntenirary.map((item, index) => {
                return {
                    ...
                    grouptouritineraryimagesList: item.grouptouritineraryimages.map((image) => ({
                        itineraryImageName: image.itineraryImageName,
                        itineraryImageUrl: image.itineraryImageUrl,
                        type: image.type,
                    })),
            ...
            try {
                setIsLoading(true);
                const response = await post(`update-group-tour-list?groupTourId=${id}`, data);
                setIsLoading(false);
    ...
    useEffect(() => {
        formik.values.itinerarydate.forEach((item, index) => {
            const textarea5 = document.getElementById(`itinerarydate[${index}].hotelAddress`);
    ...
    const handleAddInclusion = () => {
        formik.setFieldValue("inclusions", [...formik.values.inclusions, { description: "" }]);
    };
    ...
    const handleAddExclusion = () => {
        formik.setFieldValue("exclusions", [...formik.values.exclusions, { description: "" }]);
    };
            ...
            <div className="row">
                <div className="col-lg-12">
                    <form onSubmit={formik.handleSubmit} className="needs-validation">
                        ...
                        {formik.values.departure && formik.values.departure?.value != "1" && (
                            <div className="card">
                                <div className="card-body">
                                    <div className="row">
                                        <div className="col-md-12">
                                                        ...
                                                        <div className="col-md-12">
                                                            <div className="mb-2">
                                                                ...
                                                                {formik.touched.inclusions &&
                                                                    formik.touched.inclusions[index]
                                                                        ?.description &&
                                                                    formik.errors &&
                                                        ...
                                                        <div className="col-md-12">
                                                            <div className="mb-2">
                                                                ...
                                                                {formik.touched.exclusions &&
                                                                    formik.touched.exclusions[index]
                                                                        ?.description &&
                                                                    formik.errors &&
