...
const SearchGuestsGT = () => {
				...
				<>
					<div className="d-flex justify-content-center">
						<span
							...
							onClick={() => {
								navigate(
									`/group-tour-guests-details/${item?.enquiryGroupId}/${item.familyHeadGtId}`
								);
							}}
						>
	...
	const getGuestsList = async () => {
		try {
			...
			const response = await get(
				`/guest-detail-gt-list?guestId=${guestId}&perPage=${perPageItem}&page=${page}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 317) && getGuestsList();
	}, [page, perPageItem]);

	return (
		<>
			{hasComponentPermission(permissions, 317) && (
				<>
					<div className="row">
