			...
			try {
				{
					if (values.refundAmount == 0) {
						...
						const data = {
							...
							familyHeadGtId: familyHeadDetails.familyHeadGtId,
							guestId: cancelMemberDetails.guestId,
							groupGuestDetailId: cancelMemberDetails.groupGuestDetailId,
						};

						const result = await post("cancel-gt", data);
		...
		const getRefundDetails = async () => {
			try {
				...
				const response = await get(
					`refund-cancellation-gt?groupGuestDetailId=${cancelMemberDetails.groupGuestDetailId}`
				);
