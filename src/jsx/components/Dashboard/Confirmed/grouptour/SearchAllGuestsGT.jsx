...
const SearchAllGuestsGT = () => {
				...
				<>
					<div className="d-flex justify-content-center">
						<span
							...
							onClick={() => {
								navigate(
									`/group-tour-all-guests-details/${item?.enquiryGroupId}/${item.familyHeadGtId}`
								);
							}}
						>
	...
	const getGuestsList = async () => {
		try {
			...
			const response = await get(
				`/all-guest-detail-gt-list?guestId=${guestId}&perPage=${perPageItem}&page=${page}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 316) && getGuestsList();
	}, [page, perPageItem]);

	return (
		<>
			{hasComponentPermission(permissions, 316) && (
				<>
					<div className="row">
...
export default SearchAllGuestsGT;
