...
const SearchAllGuestsCT = () => {
				...
				<>
					<div className="d-flex justify-content-center">
						<span
							...
							onClick={() => {
								navigate(
									`/custom-tour-all-guests-details/${item?.enquiryCustomId}/${item.enquiryDetailCustomId}`
								);
							}}
						>
	...
	const getGuestsList = async () => {
		try {
			...
			const response = await get(
				`/all-guest-detail-ct-list?guestId=${guestId}&perPage=${perPageItem}&page=${page}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 315) && getGuestsList();
	}, [page, perPageItem]);

	return (
		<>
			{hasComponentPermission(permissions, 315) && (
				<>
					<div className="row">
...
export default SearchAllGuestsCT;
