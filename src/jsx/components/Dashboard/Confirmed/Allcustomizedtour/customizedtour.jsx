...
const ConfirmCustomizedtourCT = () => {
    ...
    const columns = [
        ...
        hasComponentPermission(permissions, 250) && {
            title: "Billing",
            render: (item) => (
                <>
                    <div className="d-flex justify-content-center">
                        <span
                            onClick={() =>
                                navigate(`/all-confirm-custom-enquiry/${item.enquiryCustomId}/`)
                            }
                        >
        ...
        hasComponentPermission(permissions, 250) && {
            ...
            render: (item) => (
                <>
                    <div className="d-flex justify-content-center">
                        <Link
                            to={`/all-confirm-custom-enquiry/${item.enquiryCustomId}/`}
                            className="btn-tick"
                        >
                            <Tooltip title="View">
                                <svg
    ...
    const getConfirmcustomlist = async () => {
        try {
            ...
            const response = await get(
                `/all-confirm-custom-list?perPage=${perPageItem}&page=${page}&tourName=${tourName}&startDate=${startDate}&endDate=${endDate}`
            );
            setIsLoading(false);
    ...
    useEffect(() => {
        hasComponentPermission(permissions, 249) && getConfirmcustomlist();
    }, [page, perPageItem]);

    return (
        <>
            {hasComponentPermission(permissions, 249) && (
                <>
                    <div className="row">
