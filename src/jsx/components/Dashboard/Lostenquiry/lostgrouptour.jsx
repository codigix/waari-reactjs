...
const LostGrouptour = () => {
	...
	const getLostlist = async () => {
		try {
			...
			const response = await get(
				`/lost-enquiry-group-tour?perPage=${perPageItem}&page=${page}&guestName=${guestName}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 106) && getLostlist();
	}, [page, perPageItem]);
	// to get the Lost group list
						...
						<div className="row page-titles mx-0 fixed-top-breadcrumb">
							   <ol className="breadcrumb">
								...
								<li className="breadcrumb-item  ">
									<Link to="/lost-grouptour">Group Tour</Link>
								</li>
							</ol>
						</div>
					</div>
					{hasComponentPermission(permissions, 106) && (
						<div className="card">
