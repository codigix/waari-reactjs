...
const LostCustomizedtour = () => {
	...
	const getLostlist = async () => {
		try {
			...
			const response = await get(
				`/all-lost-enqs-ct?perPage=${perPageItem}&page=${page}&guestName=${guestName}`
			);
			setIsLoading(false);
	...
	useEffect(() => {
		hasComponentPermission(permissions, 272) && getLostlist();
	}, [page, perPageItem]);
	// to get the Lost group list
	return (
		<>
			{hasComponentPermission(permissions, 272) && (
				<div className="row">
							...
							<div className="row page-titles mx-0 fixed-top-breadcrumb">
								<ol className="breadcrumb">
									...
									<li className="breadcrumb-item  ">
										<Link to="/all-lost-customizedtour">Customized Tour</Link>
									</li>
								</ol>
							</div>
						</div>
